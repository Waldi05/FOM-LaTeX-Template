...
\ifentrytype{incollection}{
  ...
  \iffieldundef{crossref}%
      {}% has no crossref → leave edition alone
      {\clearfield{edition}}
}{}
\ifentrytype{article}{
  \clearfield{issn}%
...
%% Anzeigen des Stichwort statt des langen Titels bei Einträgen für Sammelwerke im Literaturverzeichnis
\renewbibmacro*{maintitle+booktitle}{%
  ...
  {% *** ONLY for @incollection with crossref: print parent's shorttitle ***
    \entrydata{\thefield{crossref}}{%
      \printfield{shorttitle}%
    }%
    \newunit%
  }%
  {% *** All other cases: original standard.bbx behavior ***
...
\renewbibmacro*{publisher+location+date}{%
  ...
  {% Suppressed: location + publisher, but date still prints
    \usebibmacro{date}%
    \newunit%
  }%
  {% Original standard.bbx behavior for everything else
    \printlist{location}%
...
% Dies ist im Leitfaden von 2024 nicht mehr aktuell, siehe Seite 35.
...
% Kurzen Journal Titel statt dem langen verwenden
...
\DeclareCiteCommand{\lawcite} %Gesetze können, wie im Leitfaden verlangt ordentlich zitiert werden.
  {}
  {%
  ...
  {\multicitedelim}
  {}
\DeclareFieldFormat[legislation]{date}{%
  \iffieldundef{year}
    {}
...
\DeclareBibliographyDriver{legislation}{%
  ...
  \printtext[emph]{\printfield{title}}%
  \setunit{\addspace}%
\iffieldundef{shorttitle}{}{\printtext[emph]{(\printfield{shorttitle})}}%
  \setunit{\addspace vom \addspace}%
  \usebibmacro{date}
  \setunit{\addcomma\space}%
  \printfield{note}%
  \finentry}
